<template>
  <div class="fondo">
    <div class="container">
      <h1 class="title">Tipo de Roles</h1>
      <p class="subtitle">Administra los tipos de roles. Agrega, actualiza y elimina registros.</p>
      ...
      <div class="form-group">
        <form @submit.prevent="guardar">
          ...
          <input
            ...
            type="text"
            placeholder="Nombre del tipo de rol"
            required
          />
      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="r in roles" :key="r.id">
            ...
            <td>{{ r.nombre }}</td>
            <td>
              <button @click="editarRol(r)" class="btn btn-warning">Modificar</button>
              <button @click="eliminarRol(r.id)" class="btn btn-danger">Eliminar</button>
            </td>
          </tr>
        </tbody>
      </table>
...
import { getRoles, createRol, updateRol, deleteRol } from '@/api.js';
...
const guardar = async () => {
  try {
    if (editando.value) {
      await updateRol(editId.value, nuevo.value);
      editando.value = false;
      editId.value = null;
    } else {
      await createRol(nuevo.value);
    }
    nuevo.value = { nombre: '' };
    await cargarRoles();
  } catch (error) {
    console.error('Error al guardar tipo de rol:', error);
  }
};
